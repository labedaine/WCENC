              ...
              if (data.payload) {
				...
				$.each(data.payload, function(competition, element ) {
					...
					$.each(element.detail, function(id, detail ) {
						...
						$("[case="+cpt+"]")
									.append("<div classement="+cpt+" class='col-md-2 col-xs-1 center' style='padding:10px;max-width:30px'>"+(cpt+1)+ "/</div>")
									.append("<div classement="+cpt+" class='col-md-2 col-xs-1 center' style='padding:10px;max-width:30px'>" + detail.points + "</div>")
